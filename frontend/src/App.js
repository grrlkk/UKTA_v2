...
const TextInput = () => {
	...
	return (
		<div className='grid grid-cols-1 gap-4'>
			...
			<div className='grid grid-cols-1 gap-4 p-3 rounded-xl bg-slate-100 shadow'>
				<div className='flex justify-between gap-2 text-sm shrink'>
					<input
						...
						className="
									...
									file:cursor-pointer
									bg-slate-200 hover:bg-slate-300 rounded-xl grow sm:grow-0 shrink transition-all ease-in-out w-1/3
								"
						disabled={uploadInProgress}
					/>

					<button
						className={`flex-shrink-0 p-2 pr-4 flex gap-2 bg-red-400 text-white rounded-xl hover:bg-red-500 group`}
				...
				<div className='rounded-xl bg-white has-[:focus]:ring-2 has-[:focus]:ring-slate-500'>
					<textarea
						value={inputValue}
			...
			<div className='flex justify-end text-center gap-2 text-sm whitespace-nowrap'>
				{uploadInProgress ?
					<button className='p-2 pr-4 flex gap-1 grow sm:grow-0 bg-slate-500 text-white rounded-xl border-[1px] border-gray-300 cursor-not-allowed shadow'>
					...
					inputValue === '' ?
						<button className='p-2 pr-4 flex gap-1 grow sm:grow-0 bg-slate-500 text-white justify-center rounded-xl border-[1px] border-gray-300 cursor-not-allowed shadow'>
						...
						<>
							<Link to='/loading' className={`grow sm:grow-0 px-4 py-2 bg-slate-500 text-white rounded-xl hover:bg-slate-600 shadow`} onClick={() => handleAnalysis('morpheme')}>
								형태소 분석
							</Link>

							<Link to='/loading' className={`grow sm:grow-0 px-4 py-2 bg-slate-500 text-white rounded-xl hover:bg-slate-600 shadow`} onClick={() => handleAnalysis('cohesion')}>
								응집도 분석
							</Link>
						</>}
			</div>
