		...
		<div className='grid grid-cols-1 gap-4'>
			...
			<Pagination componentArray=
				{morphemeResult.sort((a, b) => -a._id.localeCompare(b._id)).map((item, index) => (
					<div key={index} className={`p-4 h-fit rounded-xl overflow-auto w-full shadow relative transition-all ${selectedFile === index ? 'bg-slate-100' : 'bg-slate-50 hover:bg-slate-100'}`}>
						...
						<div className='absolute top-3 right-3 flex gap-2 text-sm '>
							<div className='flex divide-x-[1px]'>
								...
								<button className={`grow sm:grow-0 pr-4 py-2 pl-2 bg-slate-500 text-white rounded-xl rounded-l-none hover:bg-slate-600 flex flex-nowrap gap-2`} onClick={() => handleFileDownload(item, "txt")}>
									txt
								</button>
							</div>
