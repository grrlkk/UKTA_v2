...
const findRangesByNormalizedMatch = (source, phrase) => {
  ...
  while (true) {
    ...
    const endOrig = map[idx + pnorm.length - 1] + 1;
...
/* =========================================================
   ▶ AI 피드백 렌더러
========================================================= */
function AIFeedbackRenderer({ aiJson, aiMd, isLoading, language }) {
  const issues = Array.isArray(aiJson?.issues) ? aiJson.issues : [];

  if (aiJson && (aiJson.summary || issues.length > 0 || aiJson.one_liner)) {
    return (
      <div className="fb-ai-block">
        {aiJson.summary && (
          <section className="fb-ai-summary">
            <div className="fb-ai-label">
              {language === "en" ? "Overall Comment" : "종합 코멘트"}
            </div>
            <p>{aiJson.summary}</p>
          </section>
        )}
        ...
        {issues.length > 0 && (
          <>
            <div className="fb-ai-section-title">
              {language === "en" ? "Focused Revision Points" : "집중 첨삭 포인트"}
            </div>

            <div className="fb-ai-issue-list">
              {issues.map((item, idx) => {
                const before = item?.edit?.before || "";
                const after = item?.edit?.after || "";

                return (
                  <article
                    key={`${item?.metric_link || "issue"}-${idx}`}
                    className="fb-issue-card"
                  >
                    <div className="fb-issue-head">
                      <div className="fb-issue-head-main">
                        <span className="fb-issue-no">{idx + 1}</span>
                        <h3 className="fb-issue-title">
                          {item?.phenomenon ||
                            (language === "en" ? "Revision Point" : "첨삭 포인트")}
                        </h3>
                      </div>

                      {item?.rubric && (
                        <span className="fb-rubric-chip">{item.rubric}</span>
                      )}
                    </div>

                    {(before || after) && (
                      <div className="fb-compare-card">
                        <div className="fb-compare-row">
                          <div className="fb-compare-label">
                            {language === "en" ? "Before" : "기존"}
                          </div>
                          <div className="fb-compare-text">
                            {before || "-"}
                          </div>
                        </div>

                        <div className="fb-compare-row">
                          <div className="fb-compare-label is-after">
                            {language === "en" ? "After" : "수정"}
                          </div>
                          <div className="fb-compare-text is-after">
                            {after || "-"}
                          </div>
                        </div>
                      </div>
                    ...
                    {item?.suggestion && (
                      <div className="fb-ai-box fb-ai-box-accent">
                        <div className="fb-ai-box-title">
                          {language === "en" ? "Try this" : "이렇게 해보세요"}
                        </div>
                        <p>{item.suggestion}</p>
                      </div>
                    )}
        ...
        {aiJson.one_liner && (
          <section className="fb-one-liner">
            <div className="fb-ai-label">
              {language === "en" ? "One-line Cheer" : "한 줄 응원"}
            </div>
            <p>{aiJson.one_liner}</p>
          </section>
        )}
      </div>
    );
...
export default function Feedback() {
  ...
  useEffect(() => {
    ...
    async function runFeedbackGeneration() {
      ...
      try {
        ...
        const requestBody = {
          original_text: rs.text ?? originalText,
          feat29: rs.feat29 ?? {},
          rubric_scores: {
            grammar: rs.grammar,
            ...
            Topic_relevance: rs.Topic_relevance,
          },
        ...
        setAiMd(data.final_markdown || data.final_md || "");
        setAiJson(parseAiJson(data.ai_json) || null);
      } catch (e) {
        if (!cancelled) {
  ...
  }, [essayScore, originalText, setIsLoading]);
  ...
  return (
    <section className="fb-wrap">
      <style>{`
        ...
        .fb-readonly::-webkit-scrollbar{
          width:8px;
          height:8px;
        }

        .fb-readonly::-webkit-scrollbar-thumb{
        ...
        .fb-md tbody tr:last-child td{
          border-bottom:0;
        }

        .fb-md th:first-child,
        .fb-md td:first-child{
          width:96px;
          min-width:96px;
          max-width:96px;
          white-space:nowrap;
        ...
        .fb-md th:nth-child(2),
        .fb-md td:nth-child(2){
          text-align:center;
          vertical-align:middle;
          line-height:1.8;
          white-space:normal;
