...
// // 표/총점 반영 키(이미지 순서)
// const GROUPS = [
//   { keys: ["topic_clarity", "narrative", "originality"] },                    // 내용 (이제 15 + 15 + 9 + 기본5 = 44)
//   { keys: ["intra_paragraph_structure", "inter_paragraph_structure"] },       // 조직 (30 = 15*2)
//   { keys: ["grammar", "vocabulary", "sentence_expression"] },                 // 표현 (20 = 6*3 + 기본2)
// ];
// const ORDERED_KEYS = GROUPS.flatMap(g => g.keys);
...
// // 최대 점수 총합(제외 항목 제외, 가중치 반영 전 상한 합)
// const TOTAL_MAX_INCLUDED = ORDERED_KEYS.reduce((acc, k) => acc + (MAX_SCORES[k] || 0), 0); // 87 (창의성 9 반영)

// // 기본점수: 내용 +5, 표현 +2
// const TOTAL_BONUS = 7;
// const TOTAL_MAX_WITH_BONUS = TOTAL_MAX_INCLUDED + TOTAL_BONUS; // 94
...
// 						  {result.map((essayScore, idx) => {
// 							const adj = adjustScore(key, essayScore[key]);   // 표시는 가중 점수
// 							const maxAdjRef = MAX_SCORES[key];               // 분모는 원만점(6, 15, 창의성 9)
// 							const maxInCol = Math.max(...result.map(ess => adjustScore(key, ess[key]))); // 하이라이트 기준은 가중
// 							return (
// 								<span
// 								key={idx}
