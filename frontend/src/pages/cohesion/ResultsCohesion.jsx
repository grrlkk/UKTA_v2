// /home/ukta/KorCAT-web_v2/frontend/src/pages/cohesion/ResultsCohesion.jsx
...
const ResultCoh = ({ resultId, darkMode }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("백엔드 디버깅 결과:", data.results); // key 수정
      setItem(data);
    } catch (error) {
      console.error(error);
    }
...
const ResultsCoh = ({ darkMode }) => {
  ...
  const handleGoFeedback = async (item) => {
    try {
      // 1) 상세에서 최신 결과 재조회 (항상 results.essay_score.feat29 확보)
      const res = await fetch(`${process.env.REACT_APP_API_URI}/korcat/cohesion/${item._id}`);
      const detail = await res.json();

      const results = detail?.results ?? item?.results ?? null;
      const rawEssay = results?.essay_score ?? detail?.essay_score ?? null; // 구 스키마 호환

      // 2) ✅ original_text(필수) 보장을 위해 text 주입
      const text =
        rawEssay?.text ??
        detail?.contents ??
        item?.contents ??
      ...
      const essayScore = rawEssay ? { ...rawEssay, text } : null;

      // 3) payload 빌드 (top-level essay_score도 함께 넣어 호환 보장)
      const payload = {
        _id: detail?._id ?? item._id,
        filename: detail?.filename ?? item.filename,
        contents: detail?.contents ?? item.contents ?? "",
        upload_date: detail?.upload_date ?? item.upload_date,
        process_time: detail?.process_time ?? item.process_time,
        results: results ? { ...results, essay_score: essayScore }
                         : (essayScore ? { essay_score: essayScore } : null),
        essay_score: essayScore, // ✅ Feedback.jsx가 top-level만 읽어도 동작
        original_text: text,
      };

      sessionStorage.setItem('feedback_payload', JSON.stringify(payload));
                ...
                <div className={`absolute top-4 right-4 flex gap-2 text-sm pl-4`}>
                  ...
                  <button
                    className="
                      ...
                      px-3 py-2 rounded-lg font-medium text-white shadow-md
                    ...
                    aria-label={LABELS?.feedback?.[language] ?? 'Feedback'}
                  >
