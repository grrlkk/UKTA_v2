...
class Multi30k:
    def __init__(
        self, lang=("en", "de"), max_seq_len=256, unk_idx=0, pad_idx=1, sos_idx=2, eos_idx=3, vocab_min_freq=2
    ):
        ...
        self.sos = "<sos>"
        self.eos = "<eos>"
        self.specials = {self.unk: self.unk_idx, self.pad: self.pad_idx, self.sos: self.sos_idx, self.eos: self.eos_idx}
    ...
    def build_vocab(self, cache_dir=".data"):
        ...
        vocab_src = build_vocab_from_iterator(
            yield_tokens(is_src=True), min_freq=self.vocab_min_freq, specials=self.specials.keys()
        )
        ...
        vocab_tgt = build_vocab_from_iterator(
            yield_tokens(is_src=False), min_freq=self.vocab_min_freq, specials=self.specials.keys()
        )
    ...
    def build_tokenizer_tgt(self, lang):
        ...
        spacy_lang_dict = {"en": "ko_core_news_sm", "de": "ko_core_news_sm"}
    ...
    def translate(self, model, src_sentence: str, decode_func):
        ...
        tgt_tokens = (
            decode_func(model, src, max_len=num_tokens * 10, start_symbol=self.sos_idx, end_symbol=self.eos_idx)
            .flatten()
            .cpu()
            .numpy()
        )
