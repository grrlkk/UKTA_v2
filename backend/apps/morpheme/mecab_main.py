...
def main(args):
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer=optimizer, verbose=True, factor=SCHEDULER_FACTOR, patience=SCHEDULER_PATIENCE
    )
    ...
    for epoch in range(1401, N_EPOCH):
        ...
        logging.info(
            DATASET.translate(
                model,
                "2012 년 세계 자연 보전 총회 ( World Conservation Congress ) 유치 에 성공 한 제주도 가 세계 환경 수도 조성 에 잰 걸음 을 내딛 었 다 .",
                greedy_decode,
            )
        )
        ...
        logging.info(
            DATASET.translate(
                model, "한 해설 위원 은 “ 일반 적 으로 판타지 스타 ( Fantasista ) 라 하 면 드리블 , 슛 , 패스 능력 을 두루 갖춘 선수 다 .", greedy_decode
            )
        )
